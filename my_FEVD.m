...
cd 'C:\Users\Kristyna\Disk Google\Diplomka\Code_data_revision'
...
% st_dev = std(Y,1)'; %column of standard deviations for all variables
Ytemp = standardize(Y); 
st_dev = ones(size(Y,2),1);
...
t = size(Y1{1,1},1);
  ...
  xlswrite('adf_kpss_transformed',Y1{2,1},'Sheet1','A1');
...
vars_nonst{1,1} = [1 20 18 24]';
vars_nonst{2,1} = [1 20 2 18 51 24]';
...
time = [2008+9/12 2013+11/12 2017+3/12]'; %times in which the large TVP-VAR wins for the baseline model 
...
FEVD_mean = cell(length(time),1); %this will store mean FEVD for each of the chosen times
for i=1:length(time)
    FEVD_mean{i,1} = cell(length(vars{best_model(i,1),1}),1);
    for j=1:length(vars{best_model(i,1),1})
        FEVD_mean{i,1}{j,1} = squeeze(mean(FEVD{i,1}{j,1}));
    end
end
